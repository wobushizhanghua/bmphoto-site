    ...
    <tbody>
      <tr>
        <td class="btns_container">
          <?php echo WDFHTML::jfbutton(__('Insert', 'wde'), '', '', 'onclick="onBtnInsertClick(event, this, \'' .  $id . '\');"'); ?>
        </td>
      </tr>
    </tbody>
  </table>
    ...
    <tbody>
      ...
      if ($rows) {
        for ($i = 0; $i < count($rows); $i++) {
          ...
          ?>
      <tr id="tr_<?php echo $row->term_id; ?>" class="row<?php echo $i % 2; ?> <?php echo $alternate; ?>" itemId="<?php echo $row->term_id; ?>" itemName="<?php echo $row->name; ?>" itemLevel="<?php echo $row->level; ?>" itemTree = "<?php echo $row->tree; ?>">
        <td class="col_num">
          <?php echo $i + 1; ?>
        </td>
        <td class="col_checked check-column">
          <input id="cb<?php echo $row->term_id; ?>" name="cid[]" value="<?php echo $row->term_id; ?>" type="checkbox" class="wde_check" />
        </td>
        <td class="col_id">
          <?php echo $row->term_id; ?>
        </td>
        <td class="col_name">
          <?php echo str_repeat('<span class="gi">|&mdash;</span>', $row->level - 1) ?>
          <a href="#" onclick="onItemClick(event, this, '<?php echo $id; ?>');"><?php echo $row->name; ?></a>
        </td>
      </tr>
          <?php
        }
